...
function ENT:Draw( )
	...
	if ( LocalPlayer( ):GetPos( ):Distance( self:GetPos( ) ) < 256 ) then
                ...
                if self:GetBoilerTimer( ) > 0 then
                        draw.WordBox( 6, 0, -150, "Timer: " .. math.Round( self:GetBoilerTimer( ) ) .. " seconds", "BarleyProcFont", Color( 0, 0, 0, 150 ), Color( 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
                end

                if self:GetBoilerTimer( ) == 0 then
                        draw.WordBox( 6, 0, -150, "Hit E on me.", "BarleyProcFont", Color( 0, 0, 0, 150 ), Color( 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
                end
                cam.End3D2D( )
	end
end
