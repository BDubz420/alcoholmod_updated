...
function ENT:Draw( )
	...
	if ( LocalPlayer( ):GetPos( ):Distance( self:GetPos( ) ) < 256 ) then
                ...
                if self:GetNeutralSpiritTimer( ) > 0 then
                        draw.WordBox( 6, 0, -150, "Timer: " .. math.Round( self:GetNeutralSpiritTimer( ) ) .. " seconds", "BarleyProcFont", Color( 0, 0, 0, 150 ), Color( 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
                end

                if self:GetFilled( ) then
                        draw.WordBox( 6, 0, -150, "Pour the soaked botanical into a pot still.", "BarleyProcFont", Color( 0, 0, 0, 150 ), Color( 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
                end
